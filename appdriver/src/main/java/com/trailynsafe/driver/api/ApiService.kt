package com.trailynsafe.driver.api
...
data class LoginResponse(val chofer: Chofer, val token: String, val message: String)

data class ErrorResponse(val error: String, val messages: Map<String, List<String>>? = null)
...
interface ApiService {
    ...
    suspend fun getRutasChofer(
            @Header("Authorization") token: String
    ): Response<List<com.trailynsafe.driver.ui.trips.Viaje>>
    ...
    /**
     * Obtener viajes del chofer con lógica tipo alarma Respuesta incluye: viajes_hoy, viajes_otros,
     * estado_efectivo calculado en tiempo real
     */
    @GET("api/chofer/viajes")
    suspend fun getViajesChofer(
            @Header("Authorization") token: String
    ): Response<com.trailynsafe.driver.ui.trips.ViajesResponse>

    @POST("api/chofer/viajes/{viaje_id}/abrir-confirmaciones")
    suspend fun abrirConfirmaciones(
            @Header("Authorization") token: String,
            @Path("viaje_id") viajeId: Int
    ): Response<Map<String, Any>>

    @POST("api/chofer/viajes/{viaje_id}/programar")
    suspend fun programarViaje(
            @Header("Authorization") token: String,
            @Path("viaje_id") viajeId: Int
    ): Response<Map<String, Any>>
    ...
    suspend fun cerrarConfirmaciones(
            @Header("Authorization") token: String,
            @Path("viaje_id") viajeId: Int,
            @Body gps: Map<String, Double>
    ): Response<Map<String, Any>>
    ...
    // Soporte Tickets
    @GET("api/chofer/soporte/tickets")
    suspend fun getTicketsSoporte(
            @Header("Authorization") token: String
    ): Response<List<SoporteTicket>>

    @POST("api/chofer/soporte/tickets")
    suspend fun crearTicketSoporte(
            @Header("Authorization") token: String,
            @Body request: CrearTicketRequest
    ): Response<SoporteTicket>

    @GET("api/chofer/soporte/tickets/{id}")
    suspend fun getDetalleTicket(
            @Header("Authorization") token: String,
            @Path("id") ticketId: Int
    ): Response<SoporteTicket>

    @POST("api/chofer/soporte/tickets/{id}/mensajes")
    suspend fun enviarMensajeSoporte(
            @Header("Authorization") token: String,
            @Path("id") ticketId: Int,
            @Body request: EnviarMensajeRequest
    ): Response<EnviarMensajeResponse>

    @DELETE("api/chofer/soporte/tickets/{id}")
    suspend fun eliminarTicketSoporte(
            @Header("Authorization") token: String,
            @Path("id") ticketId: Int
...
data class QRScanRequest(val codigo_qr: String, val latitud: Double, val longitud: Double)
...
data class SoporteTicket(
        ...
        val created_at: String,
        val mensajes: List<SoporteMensaje>? = null
)
...
data class EnviarMensajeRequest(val mensaje: String)

data class EnviarMensajeResponse(val message: String, val ticket: SoporteTicket)
