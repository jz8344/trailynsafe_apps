package com.trailynsafe.driver.ui.trips
...
data class Unidad(val id: Int, val matricula: String?, val capacidad: Int?)
...
class TripsAdapter(private val onTripClick: (Viaje) -> Unit) :
        ...
        fun bind(viaje: Viaje) {
            ...
            badgeEstado.text =
                    when (estadoEfectivo) {
                        ...
                        "programado" ->
                                "⏳ ${viaje.estado_datos?.confirmaciones_abren ?: "Esperando"}"
                        "en_confirmaciones" ->
                                "📝 Confirmando (${viaje.estado_datos?.minutos_para_salida?.toInt() ?: ""}min)"
                        "confirmado" ->
                                "✅ Listo - ${viaje.estado_datos?.minutos_para_interactuar?.toInt() ?: 0}min"
                        "interactuable" -> "🟢 ¡LISTO!"
            ...
            val alpha =
                    if (viaje.interactuable || estadoEfectivo == "en_confirmaciones") 1.0f else 0.6f
            ...
            when (estadoBd) {
                ...
                "en_confirmaciones" -> {
                    ...
                    if (confirmacionesHoy >= cupoMinimo) {
                        ...
                        btnAccion.text =
                                "⏳ Esperando confirmaciones ($confirmacionesHoy/$cupoMinimo)"
            ...
            itemView.setOnClickListener { onTripClick(viaje) }

            btnAccion.setOnClickListener { onTripClick(viaje) }
        }
    }
}
