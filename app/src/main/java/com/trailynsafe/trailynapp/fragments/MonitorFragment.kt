...
import com.trailynsafe.app.api.ConfirmacionDetalle
import com.trailynsafe.app.api.Hijo
import com.trailynsafe.app.api.RetrofitClient
import com.trailynsafe.app.api.ViajeActivo
...
class MonitorFragment : Fragment(), OnMapReadyCallback, SensorEventListener {
    ...
    private fun abrirTracking(rutaId: Int, viajeId: Int) {
        val intent =
                Intent(
                                requireContext(),
                                com.trailynsafe.app.ui.tracking.TrackingActivity::class.java
                        )
                        .apply {
                            putExtra(
                                    com.trailynsafe.app.ui.tracking.TrackingActivity
                                            .EXTRA_RUTA_ID,
                                    rutaId
                            )
                            putExtra(
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                ...
                for (hijo in hijos) {
                    try {
                        val request =
                                com.trailynsafe.app.api.ConfirmacionRequest(
                                        hijo_id = hijo.id,
