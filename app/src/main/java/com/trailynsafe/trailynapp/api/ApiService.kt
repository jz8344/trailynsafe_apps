package com.trailynsafe.app.api
...
interface ApiService {
        ...
        @POST("register") suspend fun register(@Body request: RegisterRequest): Response<Usuario>
        ...
        suspend fun restablecerPassword(@Body body: Map<String, String>): Response<MessageResponse>
        ...
        @GET("confirmaciones/mis-confirmaciones")
        suspend fun getMisConfirmacionesList(
                @Header("Authorization") token: String
        ): Response<List<ConfirmacionDetalle>>
        ...
        @retrofit2.http.PUT("confirmaciones/{confirmacionId}/direccion")
        suspend fun actualizarDireccionConfirmacion(
                @retrofit2.http.Path("confirmacionId") confirmacionId: Int,
                @Header("Authorization") token: String,
                @Body body: ConfirmacionRequest
        ): Response<ConfirmacionResponse>

        @GET("viajes/mis-viajes-activos")
        suspend fun getMisViajesActivos(
                @Header("Authorization") token: String
        ): Response<List<ViajeActivo>>

        @GET("tracking/ruta/{rutaId}")
        suspend fun getTrackingRuta(
                @retrofit2.http.Path("rutaId") rutaId: Int,
                @Header("Authorization") token: String
        ): Response<TrackingResponse>

        @GET("escuelas-activas") suspend fun getEscuelasActivas(): Response<List<EscuelaSimple>>

        @POST("hijos")
        suspend fun registrarHijo(
                @Header("Authorization") token: String,
                @Body body: Map<String, @JvmSuppressWildcards Any>
        ): Response<Hijo>

        @retrofit2.http.PUT("hijos/{hijoId}")
        suspend fun actualizarHijo(
                @retrofit2.http.Path("hijoId") hijoId: Int,
                @Header("Authorization") token: String,
                @Body body: Map<String, @JvmSuppressWildcards Any>
        ): Response<Hijo>
}
...
data class ConfirmacionResponse(val message: String, val confirmacion: ConfirmacionCreada)
...
data class LoginResponse(val usuario: Usuario, val token: String)
...
// Google Authentication
data class GoogleAuthRequest(val id_token: String, val device_name: String = "android-device")
...
data class GoogleAuthData(val usuario: GoogleUsuario, val token: String)
...
// Profile Management
data class EditarPerfilResponse(val message: String, val usuario: PerfilUsuario)
...
data class ValidarPasswordResponse(val message: String, val token_validacion: String)

data class UnidadInfo(val matricula: String, val modelo: String?, val imagen: String?)

data class ChoferInfo(val nombre: String, val apellidos: String?, val telefono: String?)

data class EscuelaInfo(val nombre: String, val direccion: String?)

data class HijoViajeInfo(val id: Int, val nombre: String, val escuela: String?)

data class SolicitudViajeResponse(val message: String, val solicitud: SolicitudViaje)
...
data class MisSolicitudesResponse(val solicitudes: List<SolicitudViajeDetalle>)
...
data class ViajesDisponiblesResponse(val viajes: List<ViajeDisponible>)
...
data class ViajeDisponible(
        ...
        val tipo_viaje: String? = null,
        val nombre: String? = null,
        val turno: String? = null,
        val fecha_viaje: String? = null,
        val hora_salida_programada: String? = null,
        val hora_salida_real: String? = null,
        val hora_llegada_estimada: String? = null,
        val hora_llegada_real: String? = null,
        val estado: String? = null,
        val confirmaciones_actuales: Int? = null,
        val cupo_maximo: Int? = null,
        val escuela: EscuelaDetalle? = null,
        val chofer: ChoferDetalle? = null,
        val unidad: UnidadDetalle? = null
)
...
data class MisConfirmacionesResponse(val confirmaciones: List<ConfirmacionDetalle>)
...
data class ConfirmacionDetalle(
        ...
        val hijo_id: Int,
        val padre_id: Int,
        val estado_confirmacion: String? = null,
        val estado: String? = null,
        val latitud: Double?,
        val longitud: Double?,
        val direccion_formateada: String?,
        val direccion_recogida: String? = null,
        val fecha_confirmacion: String?,
...
data class ViajeActivo(
        ...
        val unidad: UnidadSimple?,
        val chofer: ChoferSimple?,
        val confirmaciones_usuario: List<ConfirmacionDetalle>?
)

data class UnidadSimple(val matricula: String?)

data class ChoferSimple(val id: Int, val nombre: String)

data class EscuelaSimple(val id: Int, val nombre: String)

data class TrackingResponse(
        val ruta: TrackingRutaInfo?,
        val viaje: TrackingViajeInfo?,
        val chofer: TrackingChoferInfo?,
        val ubicacion_actual: TrackingUbicacion?,
        val parada_actual: TrackingParada?,
        val paradas: List<TrackingParada>?
)

data class TrackingRutaInfo(
        val id: Int,
        val estado: String?,
        val polyline: String?,
        val distancia_total_km: Double?,
        val tiempo_estimado_min: Double?
)

data class TrackingViajeInfo(
        val id: Int,
        val escuela: String?,
        val unidad: String?,
        val latitud_destino: Double?,
        val longitud_destino: Double?,
        val direccion_destino: String?
)

data class TrackingChoferInfo(val id: Int, val nombre: String)

data class TrackingUbicacion(
        val latitud: Double,
        val longitud: Double,
