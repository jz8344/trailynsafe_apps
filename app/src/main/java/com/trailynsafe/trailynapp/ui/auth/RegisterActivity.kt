package com.trailynsafe.app.ui.auth
...
import com.trailynsafe.app.api.GoogleAuthRequest
import com.trailynsafe.app.api.RetrofitClient
import com.trailynsafe.app.auth.GoogleAuthManager
...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun performRegister() {
        ...
        lifecycleScope.launch {
            try {
                val request =
                        com.trailynsafe.app.api.RegisterRequest(
                                nombre = nombre,
                                apellidos = apellidos,
                                telefono = telefono,
                                correo = email,
                ...
                val response =
                        com.trailynsafe.app.api.RetrofitClient.apiService.register(request)
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    val msg = com.trailynsafe.app.utils.InputValidator.parseServerError(raw)
                    showError(msg)
                }
            } catch (e: Exception) {
    ...
    private fun handleGoogleAuthResult(result: GoogleAuthManager.GoogleAuthResult) {
        when (result) {
            ...
            is GoogleAuthManager.GoogleAuthResult.Error -> {
                val msg =
                        com.trailynsafe.app.utils.InputValidator.parseServerError(result.message)
                showError(msg)
            }
